...
public class IESCipher extends CipherSpi
{
					...
					throws InvalidAlgorithmParameterException, InvalidKeyException
	{
		...
		if (engineSpec == null)
		{
			if (engine.getCipher() == null)
			{
				this.engineSpec = new IESParameterSpec(null, null, 128);
			}
			else if ((engine.getCipher().getUnderlyingCipher().getAlgorithmName() == "DES") ||
					(engine.getCipher().getUnderlyingCipher().getAlgorithmName() == "RC2") ||
					(engine.getCipher().getUnderlyingCipher().getAlgorithmName() == "RC5-32") || 
					(engine.getCipher().getUnderlyingCipher().getAlgorithmName() == "RC5-64"))
			{
				this.engineSpec = new IESParameterSpec(null, null, 64, 64);
			}
			else if (engine.getCipher().getUnderlyingCipher().getAlgorithmName() == "SKIPJACK")	
			{
				this.engineSpec = new IESParameterSpec(null, null, 128, 80);
			}
			else if (engine.getCipher().getUnderlyingCipher().getAlgorithmName() == "GOST28147")
			{
				this.engineSpec = new IESParameterSpec(null, null, 128, 256);
			}
			else
			{
				this.engineSpec = new IESParameterSpec(null, null, 128, 128);
			}
		}
