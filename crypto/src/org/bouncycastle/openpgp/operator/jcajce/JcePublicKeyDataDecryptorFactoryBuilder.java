    ...
    public PublicKeyDataDecryptorFactory build(final PGPPrivateKey privKey)
    {
         return new PublicKeyDataDecryptorFactory()
         {
             public byte[] recoverSessionData(int keyAlgorithm, BigInteger[] secKeyData)
                 throws PGPException
             {
                 Cipher c1 = getKeyCipher(keyAlgorithm);

                 try
                 {
                     c1.init(Cipher.DECRYPT_MODE, keyConverter.getPrivateKey(privKey));
                 }
        ...
        try
        {
            switch (algorithm)
            {
                ...
                case PGPPublicKey.RSA_GENERAL:
                    return helper.createCipher("RSA/ECB/PKCS1Padding");
                case PGPPublicKey.ELGAMAL_ENCRYPT:
                case PGPPublicKey.ELGAMAL_GENERAL:
                    return helper.createCipher("ElGamal/ECB/PKCS1Padding");
                default:
