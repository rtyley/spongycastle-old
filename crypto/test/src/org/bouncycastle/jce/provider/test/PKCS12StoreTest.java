    ...
    extends SimpleTest
{
    ...
    private void basicStoreTest(PrivateKey privKey, Certificate[] chain, String type)
        throws Exception
    {
        KeyStore store = KeyStore.getInstance(type, BouncyCastleProvider.PROVIDER_NAME);
        ...
        throws Exception
    {
        ...
        KeyStore store1 = KeyStore.getInstance("PKCS12", BouncyCastleProvider.PROVIDER_NAME);

        store1.load(new ByteArrayInputStream(store1data), passwd);

        KeyStore store2 = KeyStore.getInstance("PKCS12", BouncyCastleProvider.PROVIDER_NAME);
        ...
        throws Exception
    {
        ...
        data = PKCS12Util.convertToDefiniteLength(pkcs12, passwd, BouncyCastleProvider.PROVIDER_NAME);
        kS.load(new ByteArrayInputStream(data), passwd); //check MAC
